...
async function upload(final: boolean = false): Promise<void> {
  ...
  let sendPlaybackBytes =
    config.lean === false &&
    playbackBytes > 0 &&
    (playbackBytes < Setting.MaxFirstPayloadBytes ||
...
function stringify(encoded: EncodedPayload): string {
  return encoded.p.length > 0
    ? `{"e":${encoded.e},"a":${encoded.a},"p":${encoded.p}}`
    : `{"e":${encoded.e},"a":${encoded.a}}`;
}
...
function check(xhr: XMLHttpRequest, sequence: number): void {
  ...
  if (xhr && xhr.readyState === XMLReadyState.Done && transitData) {
    ...
    if (
      (xhr.status < 200 || xhr.status > 208) &&
      transitData.attempts <= Setting.RetryLimit
    ) {
...
function delay(): number {
  ...
  let gap =
    config.lean === false && discoverBytes > 0
      ? Setting.MinUploadDelay
      : envelope.data.sequence * config.delay;
...
function response(payload: string): void {
  let key =
    payload && payload.length > 0 ? payload.split(" ")[0] : Constant.Empty;
  switch (key) {
    case Constant.End:
